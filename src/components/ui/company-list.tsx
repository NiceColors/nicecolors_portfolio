...
export const Companyli: React.FC<CompanyliProp> = ({
  ...
  const periodDurationYear = dayjs(period.end).diff(
    dayjs(period.start),
    "year",
  );
  const periodDurationMonth =
    dayjs(period.end).diff(dayjs(period.start), "month") -
    periodDurationYear * 12;

  const periodDurantionYearText =
    periodDurationYear > 0
      ? `${periodDurationYear} ano${periodDurationYear > 1 ? "s" : ""}`
      : "";
  const periodDurantionMonthText =
    periodDurationMonth > 0
      ? `e ${periodDurationMonth} mês${periodDurationMonth > 1 ? "es" : ""}`
      : "";
      ...
      <div className="flex flex-col gap-3 mt-1">
        <div className="flex gap-1">
          {positionList.map((position) => (
            <Chip key={position} radius="sm" size="sm">
              {position}
            </Chip>
          ))}
        </div>
        <div className="flex items-center gap-1 text-xs">
          ...
          <Tooltip content={periodDurationDay} placement="right">
            <p>
              {periodStart} ~{" "}
              {period.isActual ? "Atual" : dayjs(period.end).format("YYYY/MM")}
            </p>
          </Tooltip>
        </div>
      </div>
